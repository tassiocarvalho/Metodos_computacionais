...
% Integral usando o método trapezoidal composta
h = r(2) - r(1); % Tamanho do passo
integral_trapezoidal_composta = 0.5 * h * (r(1) * v_r(1) + 2 * sum(r(2:end-1) .* v_r(2:end-1)) + r(end) * v_r(end));

% Velocidade média calculada numericamente
v_media_numeric = (2 * v_max / r_0^2) * integral_trapezoidal_composta;
...
fprintf('Velocidade média (numericamente - trapezoidal composta): %.6f m/s\n', v_media_numeric);
